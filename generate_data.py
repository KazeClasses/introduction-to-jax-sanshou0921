...
class PendulumSimulation(eqx.Module):
    ...
    def simulate_pendulum(
        ...
        solver = dx.Tsit5()
        saveat = dx.SaveAt(tx=jnp.arrange(0, self.time, self.save_interval*self.dt))
    ...
    def generate_dataset(
        ...
        for i in range(n_sims):
            ...
            initial_velocity = jax.random.uniform(jax.random.PRNGKey(i * 1029423), minval=-10.0, maxval=10.0)
            solution = self.simulate_pendulum(initial_angle, initial_velocity, gravity, length)
            frames = jax.vmap(self.render_pendulum, in_axes=(0, 0, None))(solution.ys[0], solution.ys[1], length)
